...
## Filter ----
## JM 9/5/24
# Get length data/xenium
# Geschwind single cell data has the algorithm

# Subset the object so that I can use it in Rstudio
clean_curated %<>%
  arrange(desc(Major_Name), Marker) %T>%
...
txl = transcriptLengths(tx)

marker_entrezids = select(org, unique(immune_curated$Marker), keytype = "SYMBOL", columns = "ENTREZID") # TODO which genes are missing?
# 'select()' returned 1:1 mapping between keys and columns
setnames(txl, "gene_id", "ENTREZID")
marker_lengths = txl %>%
  merge(marker_entrezids, all = F) %>%
  group_by(ENTREZID) %>%
  summarise_all(max)
...
immune_curated_filtered = immune_curated %>%
  merge(Major_Expression) %T>%
